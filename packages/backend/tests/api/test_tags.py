...
class TestCreateTag:
    ...
    async def test_create_duplicate_tag_returns_400(
        ...
        await create_test_tag(
            db_session, user_id=test_user.id, name="Important", color="#FF0000"
        )
...
class TestListTags:
    ...
    async def test_list_tags_returns_user_tags(
        self, client: AsyncClient, auth_headers, db_session, test_user, test_user_2
    ) -> None:
        """Test listing returns only user's tags."""
        await create_test_tag(
            db_session, user_id=test_user.id, name="Tag 1", color="#FF0000"
        )
        await create_test_tag(
            db_session, user_id=test_user.id, name="Tag 2", color="#00FF00"
        )
        # Other user's tag
        await create_test_tag(
            db_session, user_id=test_user_2.id, name="Other's Tag", color="#0000FF"
        )
        await db_session.commit()
    ...
    async def test_list_tags_ordered_by_name(
        self, client: AsyncClient, auth_headers, db_session, test_user
    ) -> None:
        """Test tags are ordered by name."""
        await create_test_tag(
            db_session, user_id=test_user.id, name="Zebra", color="#FF0000"
        )
        await create_test_tag(
            db_session, user_id=test_user.id, name="Apple", color="#00FF00"
        )
        await db_session.commit()
...
class TestUpdateTag:
    ...
    async def test_update_tag(
        ...
        tag = await create_test_tag(
            db_session, user_id=test_user.id, name="Original", color="#FF0000"
        )
        await db_session.commit()
    ...
    async def test_update_other_users_tag_returns_404(
        self, client: AsyncClient, auth_headers, db_session, test_user, test_user_2
    ) -> None:
        """Test updating another user's tag returns 404."""
        tag = await create_test_tag(
            db_session, user_id=test_user_2.id, name="Other's", color="#FF0000"
        )
        await db_session.commit()
    ...
    async def test_update_tag_duplicate_name_returns_400(
        self, client: AsyncClient, auth_headers, db_session, test_user
    ) -> None:
...
class TestDeleteTag:
    ...
    async def test_delete_tag(
        ...
        tag = await create_test_tag(
            db_session, user_id=test_user.id, name="To Delete", color="#FF0000"
        )
        await db_session.commit()
...
class TestAddTagToPdf:
    ...
    async def test_add_tag_already_assigned_returns_400(
        self, client: AsyncClient, auth_headers, db_session, test_user
    ) -> None:
    ...
    async def test_add_other_users_tag_returns_404(
        self, client: AsyncClient, auth_headers, db_session, test_user, test_user_2
    ) -> None:
...
class TestRemoveTagFromPdf:
    ...
    async def test_remove_tag_from_pdf(
        self, client: AsyncClient, auth_headers, db_session, test_user
    ) -> None:
