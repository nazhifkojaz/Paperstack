...
async def list_annotation_sets(
    ...
    pdf = await db.scalar(
        select(Pdf).where(Pdf.id == pdf_id, Pdf.user_id == current_user.id)
    )
    if not pdf:
...
@router.post(
    "/sets", response_model=AnnotationSetResponse, status_code=status.HTTP_201_CREATED
)
async def create_annotation_set(
    ...
    pdf = await db.scalar(
        select(Pdf).where(Pdf.id == set_in.pdf_id, Pdf.user_id == current_user.id)
    )
    if not pdf:
        raise HTTPException(status_code=404, detail="PDF not found")
...
async def update_annotation_set(
    ...
    db_set = await db.scalar(
        select(AnnotationSet).where(
            AnnotationSet.id == set_id, AnnotationSet.user_id == current_user.id
        )
    )
    if not db_set:
...
async def list_annotations(
    ...
    result = await db.scalars(select(Annotation).where(Annotation.set_id == set_id))
    return result.all()


@router.post(
    "/items", response_model=AnnotationResponse, status_code=status.HTTP_201_CREATED
)
async def create_annotation(
    ...
    db_set = await db.scalar(
        select(AnnotationSet).where(
            AnnotationSet.id == ann_in.set_id, AnnotationSet.user_id == current_user.id
        )
    )
    if not db_set:
        raise HTTPException(status_code=404, detail="Annotation set not found")
