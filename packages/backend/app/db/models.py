...
class Pdf(Base):
    __tablename__ = "pdfs"
    __table_args__ = (UniqueConstraint('user_id', 'filename', name='uq_pdfs_user_filename'),)
...
class AnnotationSet(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=text("gen_random_uuid()"))
    pdf_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("pdfs.id", ondelete="CASCADE"), nullable=False)
    user_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    name: Mapped[str] = mapped_column(String(255), nullable=False, default='Default')
    color: Mapped[Optional[str]] = mapped_column(String(7), default='#FFFF00')
    source: Mapped[Optional[str]] = mapped_column(String(20), server_default=text("'manual'"))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"), onupdate=text("now()"))
...
class Annotation(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=text("gen_random_uuid()"))
    set_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("annotation_sets.id", ondelete="CASCADE"), nullable=False)
    ...
    ann_metadata: Mapped[Optional[Any]] = mapped_column("metadata", JSONB, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"), onupdate=text("now()"))
...
class Collection(Base):
    ...
    parent_id: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("collections.id", ondelete="CASCADE"))
...
class PdfCollection(Base):
    ...
    pdf_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("pdfs.id", ondelete="CASCADE"), primary_key=True)
...
class Citation(Base):
    ...
    __table_args__ = (UniqueConstraint('pdf_id', 'user_id', name='uq_citations_pdf_user'),)
...
class Share(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=text("gen_random_uuid()"))
    annotation_set_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("annotation_sets.id", ondelete="CASCADE"), nullable=False)
    shared_by: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    shared_with: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"))  # null = public link
    share_token: Mapped[str] = mapped_column(String(64), unique=True, nullable=False)
    permission: Mapped[str] = mapped_column(String(10), default='view')  # 'view' | 'comment'
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))
...
class UserApiKey(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=text("gen_random_uuid()"))
    user_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    provider: Mapped[str] = mapped_column(String(20), nullable=False)  # 'glm' | 'gemini'
...
class AutoHighlightCache(Base):
    ...
    __table_args__ = (UniqueConstraint('pdf_id', 'user_id', 'categories', name='uq_auto_highlight_cache_pdf_user_cats'),)

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=text("gen_random_uuid()"))
    pdf_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("pdfs.id", ondelete="CASCADE"), nullable=False)
    user_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    categories: Mapped[Any] = mapped_column(JSONB, nullable=False)
    status: Mapped[str] = mapped_column(String(20), nullable=False, server_default=text("'pending'"))  # 'pending' | 'complete'
...
class UserUsageQuota(Base):
    ...
    free_uses_remaining: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text("5"))
    chat_uses_remaining: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text("20"))
    explain_uses_remaining: Mapped[int] = mapped_column(Integer, nullable=False, server_default=text("20"))
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"), onupdate=text("now()"))
...
class PdfIndexStatus(Base):
    ...
    status: Mapped[str] = mapped_column(String(20), nullable=False, server_default=text("'not_indexed'"))
...
class PdfChunk(Base):
    ...
    __table_args__ = (UniqueConstraint('pdf_id', 'chunk_index', name='uq_pdf_chunks_pdf_idx'),)
...
class ChatConversation(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=text("gen_random_uuid()"))
    user_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    pdf_id: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("pdfs.id", ondelete="CASCADE"))
    collection_id: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("collections.id", ondelete="CASCADE"))
...
class ChatMessage(Base):
    ...
    content: Mapped[str] = mapped_column(Text, nullable=False)
    context_chunks: Mapped[Optional[Any]] = mapped_column(JSONB)  # [{chunk_id, page_number, snippet}]
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=text("now()"))
