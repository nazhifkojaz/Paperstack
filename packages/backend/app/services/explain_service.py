...
class ExplainService:
    ...
    async def explain_with_provider(
        ...
        await indexing_service.reset_if_stale(index_status, db)
        ...
        if index_status.status == "not_indexed":
            ...
            await indexing_service.index_pdf(pdf_row, user, index_status, db)
            await db.commit()
            logger.info("explain: indexing complete for pdf %s", pdf_row.id)

        # 2. Embed selected text as query vector
        query_vector = await embedding_service.embed_query(selected_text)
